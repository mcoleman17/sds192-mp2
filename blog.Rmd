...
FT2<-Final_table%>%
  ...
  summarize(Total= sum(Total_contr), N = sum(N), TotalC = n())%>%
  filter(cand_party_affiliation != "LIB", cand_office_state == "ME")%>%
  ggplot(aes(x= cand_party_affiliation, y = Total, fill = cand_party_affiliation)) + geom_bar(stat = "identity")+geom_text(aes(label=N), vjust=1.5, colour="black")
FT2
```
