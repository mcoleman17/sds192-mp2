...
House_graphs <- function(state_arg) {
  Political_graphs<-Joined_table%>%
    ...
    geom_bar(stat = "identity")+geom_text(aes(label=N), vjust=1.5, colour="White")+    
    ggtitle(state_arg)+ labs(caption = "Note. Numbers inside the bars are the total number of contributions to each party.")+ 
    scale_fill_manual(values=c("dodgerblue3", "red1")) + xlab("Party Affiliation") + ylab ("Total Contributions")+theme(legend.title=element_blank())
}
```

# Contribution to the 2012 House Elections in selected states 
```{r echo=FALSE, message=F, warning=F}
State<-c ("ME", "IL", "MA")
